...
export interface AdminApiKeyResolution {
  /** The SHA-256 hex hash of the API key. */
  apiKeyHash: string;
  /**
 ...
 *
 * The key is always auto-generated — operators cannot supply their own.
 *
 * Strategy:
 * 1. If opt-out flag is set (`noAdminApiKey`), return undefined (no auth).
 * 2. If a data directory exists, look for a persisted hash file
...
function generateApiKey(): { rawKey: string; hash: string } {
  ...
  const hash = sha256HexHash(rawKey);
  return { rawKey, hash };
}
