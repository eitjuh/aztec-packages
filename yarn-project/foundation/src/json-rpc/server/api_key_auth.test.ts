...
import { getApiKeyAuthMiddleware, sha256HexHash } from './api_key_auth.js';

describe('getApiKeyAuthMiddleware', () => {
  const RAW_API_KEY = 'test-api-key-for-unit-tests-1234567890abcdef';
  const API_KEY_HASH = sha256HexHash(RAW_API_KEY);
  ...
  describe('sha256HexHash', () => {
    it('produces a 64-char hex string', () => {
      const hash = sha256HexHash('hello');
      expect(hash).toMatch(/^[0-9a-f]{64}$/);
    });

    it('is deterministic', () => {
      expect(sha256HexHash('test')).toBe(sha256HexHash('test'));
    });

    it('produces different hashes for different inputs', () => {
      expect(sha256HexHash('a')).not.toBe(sha256HexHash('b'));
    });

    it('matches known SHA-256 value', () => {
      // SHA-256("hello") = 2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824
      expect(sha256HexHash('hello')).toBe('2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824');
    });
  });
});
