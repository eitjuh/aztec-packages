...
const l2Weth = await TokenContract.deploy(
  aztecWallet,
  account.address,
  "Wrapped Ether",
  "WETH",
...
const l2Dai = await TokenContract.deploy(
  aztecWallet,
  account.address,
  "Dai Stablecoin",
  "DAI",
...
const l2WethBridge = await TokenBridgeContract.deploy(
  aztecWallet,
  l2Weth.address,
  wethPortalAddress,
).send({ from: account.address });

const l2DaiBridge = await TokenBridgeContract.deploy(
  aztecWallet,
  l2Dai.address,
  daiPortalAddress,
).send({ from: account.address });
...
const l2Uniswap = await ExampleUniswapContract.deploy(
  aztecWallet,
...
const wethBalanceBefore = await l2Weth.methods
...
await aztecWallet.setPublicAuthWit(transferAction, account.address, true);

// Call swap_public on the L2 uniswap contract
const swapReceipt = await l2Uniswap.methods
  .swap_public(
    account.address,
...
// docs:start:consume_l1_messages
...
const withdrawContentHash = sha256ToField([
  ...
  Buffer.from("b1d15a28", "hex"), // keccak256 selector
  Buffer.from(
    uniswapPortalAddress.toString().slice(2).padStart(64, "0"),
    "hex",
  ),
  Buffer.from(SWAP_AMOUNT.toString(16).padStart(64, "0"), "hex"),
  Buffer.from("".padStart(64, "0"), "hex"), // address(0) for caller_on_l1
]);

// Get epoch for the swap receipt block
const swapEpoch = await rollup.getEpochNumberForCheckpoint(
  CheckpointNumber.fromBlockNumber(swapReceipt.blockNumber!),
);

// Message 1: Token bridge exit message
const exitMsgLeaf = computeL2ToL1MessageHash({
  l2Sender: l2WethBridge.address,
  l1Recipient: EthAddress.fromString(wethPortalAddress.toString()),
...
const swapMsgLeaf = computeL2ToL1MessageHash({
  ...
  l1Recipient: EthAddress.fromString(uniswapPortalAddress.toString()),
...
const daiBalance = await l2Dai.methods
  .balance_of_public(account.address)
  .simulate({ from: account.address });

const wethBalanceAfter = await l2Weth.methods
  .balance_of_public(account.address)
  .simulate({ from: account.address });

console.log(`L2 WETH balance: ${wethBalanceAfter}`);
console.log(`L2 DAI balance: ${daiBalance}`);
console.log("\nPublic swap complete!\n");
